...
export class CommunityDiscussionsService {
  ...
  public allPrevious(): Observable<communityMessage[]> {
    return this.http.get<communityMessage[]>(this.apiUrl + '/hellos/'+this.clickedDiscussion.id);
  }
  ...
  getDiscussionInfo(id:number):Observable<CommunityDiscussion>{
    return this.http.get<CommunityDiscussion>(this.apiUrl+'/api/v1/discussions/'+id)
  }
}
